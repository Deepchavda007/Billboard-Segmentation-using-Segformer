...
def find_contour_mask(mask_image_path):
    ...
    sure_bg = cv2.dilate(cv2.morphologyEx(thresholded_image, cv2.MORPH_OPEN, kernel, iterations=2), kernel, iterations=3)
...
def perspective_transformation(mask_image_path, original_image_path, advertisement_path):
    """
    ...
    for contour in contours:
        ...
        if area > 1000:
            ...
            cv2.polylines(mask_with_lines, [approx], isClosed=True, color=(0, 255, 0), thickness=2)
            ...
            transformed_mask_image, src_pts, dst_pts, max_Width, max_Height = four_point_transform(mask_image, corners)

            # Perform perspective transformation on the original image using the corners
            transformed_original_image, src_pts, dst_pts, max_Width, max_Height = four_point_transform(original_image, corners)

            # Resize the replacement image to match the transformed original image dimensions
            replacement_image_resized = cv2.resize(replacement_image, (transformed_original_image.shape[1], transformed_original_image.shape[0]))

            # Overlay the resized replacement image on the transformed original image
            transformed_original_image[0:replacement_image_resized.shape[0], 0:replacement_image_resized.shape[1]] = replacement_image_resized

            # Convert the transformed original image to RGB format
            transformed_original_image_rgb = cv2.cvtColor(transformed_original_image, cv2.COLOR_BGR2RGB)
            ...
            warped = cv2.warpPerspective(transformed_original_image_rgb, M, (original_image.shape[1], original_image.shape[0]))
            ...
            cv2.imwrite(save_path, final_image_rgb)

perspective_transformation("22_mask.png", "Test/22.png", "apple.png")
